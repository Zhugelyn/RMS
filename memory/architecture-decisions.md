...
- Presign 2026-08-19 (`phase8-presign`): `file_objects` metadata (EF + in-memory); `IObjectStoragePresigner` / MinIO SDK; `POST /v1/files/upload-intent`, confirm, download-url; size/MIME allowlist; opaque GUID keys; assistant-api `Minio__*` + depends_on; soft-fail without endpoint.
- Pack files 2026-08-20 (`phase8-pack-files`): salon/marketing MCP `files` + skill `files-store`; `FilesPackInjector` metadata inject; PackCatalog require `kb-retriever`+`files`; router/tasks empty; domain isolation tests.
- Hardening 2026-08-20 (`phase8-hardening`): TTL clamps; `IFileContentScanner` / PassThrough stub on confirm; README MinIO rotation; non-goals guard (no Direct/Apify/public bucket). **Phase 8 closed.**
