...
- [x] Caps / ApiBaseUrl=`api.vk.com` / MediaPath relative-only / token rotation / non-goals guard. (`phase6-vk-hardening`)
- [x] `dotnet test` + compose зелёные (hardening ✅).
...
6. [x] `phase6-vk-hardening` — caps, tests, token rotation notes, non-goals guard (no scrape/Apify/user-OAuth/RAG/MinIO/Direct).
...
Status: **closed** (2026-08-18). `phase7-docs` ✅; `phase7-es-compose` ✅; `phase7-rag-api` ✅; `phase7-pack-retriever` ✅; `phase7-hardening` ✅.
...
- [x] `dotnet test` + compose зелёные; no MinIO/Direct/Apify. (`phase7-hardening` ✅)
...
5. [x] `phase7-hardening` — caps, PII в логах, index isolation tests, token notes, non-goals guard (no MinIO/Direct/Apify; no mixing indexes); ES basic auth.
...
Status: **open** — `phase8-docs` ✅; `phase8-minio-compose` ✅; `phase8-presign` ✅; next=`phase8-pack-files`.
