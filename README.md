...
| **8 Files / media** | **open** (`phase8-docs` ✅; `phase8-minio-compose` ✅; `phase8-presign` ✅; next=`phase8-pack-files`) | MinIO private + presign; metadata в assistant-api (ADR-015) |
...
Object storage для фото/видео/документов (ADR-015). **Не** RAG (ES), **не** research volume (GenerateImage/VK photos остаются на disk volume до optional migration).

| Тема | Решение |
| --- | --- |
| Object store | **MinIO** private buckets only. Public bucket / CDN без TTL — запрещены |
...
- Apify, OpenAI Images
- App wiring / pack tools в `phase8-presign` (→ `phase8-pack-files`)
...
- Phase 8 presign ✅: `MINIO__*` in `.env.example` + compose; assistant-api `Minio__Endpoint/AccessKey/SecretKey` + `file_objects` + `/v1/files/*` presign. Pack tools → `phase8-pack-files`. Не из Telegram/Mini App.
...
| `MINIO__*` (Phase 8, presign ✅) | MinIO + assistant-api | Root/access via `MINIO__ROOTPASSWORD` / `MINIO__SECRETKEY` (dev defaults local-only). Buckets private. `/v1/files/*` presign. Pack MCP → `phase8-pack-files`. Не из чата/Mini App. |
